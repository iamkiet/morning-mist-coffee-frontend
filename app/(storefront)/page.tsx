    ...
    <>
      ...
      <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl px-4 sm:px-6 md:px-gutter py-12 sm:py-16 md:py-20">
          <span className="text-primary tracking-[0.3em] mb-4 sm:mb-6 block uppercase text-xs sm:text-sm font-medium">Grown in Silence</span>
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-foreground mb-6 sm:mb-8 leading-tight font-light">The Art of Living <br/> through the lens of Coffee.</h1>
          <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto mb-8 sm:mb-12 font-light">
            Morning Mist is an exploration of sensory precision. We curate rare beans and artisanal ceramics to transform your morning routine into a meditative practice.
          </p>
          <Button asChild size="lg" className="px-6 sm:px-10 py-3 sm:py-4 text-xs sm:text-sm uppercase tracking-widest rounded-none">
      ...
      <section className="py-12 sm:py-16 md:py-xl max-w-7xl mx-auto px-4 sm:px-6 md:px-gutter">
        <div className="flex flex-col mb-8 sm:mb-12 md:mb-xl">
          <span className="text-primary tracking-[0.2em] mb-2 sm:mb-4 uppercase text-xs font-medium">Curated Objects</span>
          <h2 className="text-2xl sm:text-3xl md:text-4xl text-foreground font-light">Featured Collections</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 md:gap-gutter">
          ...
          <div className="md:col-span-7 group cursor-pointer">
            ...
            <div className="flex justify-between items-end">
              <div>
                <h3 className="text-lg sm:text-xl text-foreground mb-2 font-normal">The Ethereal Series</h3>
                <p className="text-sm text-muted-foreground font-light">Limited Edition Glassware</p>
              </div>
          ...
          <div className="md:col-span-5 flex flex-col gap-gutter">
            <div className="group cursor-pointer">
              ...
              <h3 className="text-base sm:text-lg text-foreground mb-1 font-normal">Origin 01: Ethiopia</h3>
              <p className="text-sm text-muted-foreground font-light">Floral / Bergamot / Honey</p>
            </div>
            <div className="group cursor-pointer">
              <div className="relative aspect-square overflow-hidden bg-card mb-4 sm:mb-6">
                <Image
              ...
              <h3 className="text-base sm:text-lg text-foreground mb-1 font-normal">The Morning Mist Set</h3>
              <p className="text-sm text-muted-foreground font-light">Hand-thrown Stoneware</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-muted py-12 sm:py-16 md:py-xl overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-gutter grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-xl items-center">
          ...
          <div className="flex flex-col space-y-6 sm:space-y-8 order-1 md:order-2">
            <span className="text-primary tracking-[0.2em] uppercase text-xs font-medium">The Philosophy</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-foreground font-light">Crafting the <br/> Silent Moments.</h2>
            <p className="text-base sm:text-lg text-muted-foreground font-light leading-relaxed">
              Morning Mist Coffee was founded on the belief that coffee is not a commodity, but a choreography. We spend months sourcing beans from sustainable high-altitude farms, roasting them in small batches that respect the bean&apos;s inherent floral and fruit notes.
            </p>
            <p className="text-base sm:text-lg text-muted-foreground font-light leading-relaxed">
              Our ceramics are collaborations with local potters, designed to feel weighted yet effortless in your palm—a physical anchor for your most peaceful time of day.
            </p>
            <Link href="/journal" className="inline-flex items-center text-primary uppercase tracking-widest group text-xs font-medium">
              Read Our Journal
      ...
      <section className="py-12 sm:py-16 md:py-32 bg-muted/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-gutter">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 md:gap-md">
            {[
              { Icon: Leaf, title: "Sustainable Sourcing", body: "Direct trade partnerships that ensure fair compensation and ecological preservation through conscious selection." },
              { Icon: Paintbrush, title: "Artisanal Craft", body: "Every vessel is hand-finished by master potters, making each piece in your collection a unique work of quiet art." },
              { Icon: Settings2, title: "Small Batch Roast", body: "Roasting profiles developed with technical precision to highlight the complex terroir and floral notes of each origin." },
            ].map(({ Icon, title, body }) => (
              <div key={title} className="p-6 sm:p-8 md:p-12 bg-card border border-border/40 flex flex-col items-start text-left transition-all duration-500 hover:shadow-sm rounded-none">
                <div className="w-12 h-12 bg-accent flex items-center justify-center mb-8 sm:mb-10 shrink-0 rounded-none">
                  <Icon className="size-6 text-white" />
                </div>
                <h4 className="text-base sm:text-lg md:text-xl uppercase tracking-[0.15em] text-foreground mb-4 sm:mb-6 font-normal">{title}</h4>
                <div className="w-8 h-px bg-accent mb-4 sm:mb-6" />
                <p className="text-sm sm:text-base text-muted-foreground leading-relaxed tracking-wide font-light">{body}</p>
              </div>
            ))}
          </div>
        </div>
