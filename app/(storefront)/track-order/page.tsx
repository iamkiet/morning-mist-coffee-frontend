...
import { Package, Search, Clock, Truck, CheckCircle, XCircle, CreditCard } from "lucide-react";
...
const STATUS_CONFIG: Record<OrderStatus, { label: string; icon: React.ElementType; className: string }> = {
  ...
  delivered: { label: "Delivered", icon: CheckCircle,  className: "text-primary" },
    ...
    <Card>
      <CardContent className="p-5 space-y-3">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Order</p>
    ...
    <main className="max-w-2xl mx-auto pt-32 sm:pt-40 pb-16 px-4 sm:px-6 min-h-screen">
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h1 className="text-3xl font-light text-foreground">Track Your Order</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-3 mb-8">
        <Input
          ...
          className="flex-1"
        />
        <Button type="submit" disabled={isLoading} className="gap-2 uppercase tracking-widest text-xs">
