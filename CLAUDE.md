...
**Products are variant-based, not flat.** `Product` only carries identity/copy (`slug`, `name`, `description`, `image`); price, SKU, stock and expiry live on `Product.variants: ProductVariant[]`. There is no more `origin`/`tastingNotes`/`price`/`stockQuantity`/`productTypeId` on `Product` — the backend dropped them along with the `product_type` concept in favor of `product_categories` (hierarchical) and an EAV `product_properties` model. Use `lib/product-variants.ts` helpers (`getDefaultVariant`, `getPriceRange`, `getTotalStock`) instead of reading a single price/stock field. `Product.categoryIds`/`categoryNames` are now returned on every read path (`GET /products`, `/products/:id`, `/products/slug/:slug`, and chat `items`) — `categoryNames` is the display label ("dòng cà phê", e.g. Arabica/Robusta) rendered as a small tag on `ProductCard` (shop grid and `ChatWidget`'s product cards, since the widget reuses `ProductCard`); resolve it from `categoryNames`, never by calling `GET /api/v1/product-categories` from a storefront/customer context — that endpoint is admin/staff-only and 401s for anonymous visitors. The admin edit dialog still doesn't re-display/preserve existing category assignments on edit (category selection only happens at product creation) — the data is available now, that's just not wired up yet.
