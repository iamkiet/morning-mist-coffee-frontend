...
Access and refresh tokens live entirely in httpOnly cookies set by the backend — `AuthProvider` (in `lib/auth-context.tsx`) never sees the token values, only `csrfToken` and the `user` object. `AuthProvider` itself calls `ensureSession()` once on mount, for every page (not just auth-gated ones) — the access-token cookie is cross-origin (backend on a different domain), so FE JS can't read it to decide locally whether a session exists; a single `/me` check on load is the only way to know, and it's what keeps `Nav`'s account icon (filled vs outline, based on `user`) correct even on a hard reload of a public page. `ensureSession()` calls `fetchMe()` once; since the access-token cookie survives a reload, this is usually a single request — if it's expired, `authFetch`'s built-in 401→refresh→retry handles it transparently. `ensureSession()` is idempotent (guarded by a ref) and is still exposed from the context for a route that wants to call it explicitly (e.g. right after a redirect), but no route needs to anymore since the provider always does it. `isLoading` stays `true` until an attempt settles.

All auth endpoints live in `lib/api/auth.ts` — never call them with a bare `fetch()`. FE never manually attaches an `Authorization` header or reads `csrf_token`/`access_token` cookies via `document.cookie` — the backend is on a different origin, so those cookies are never readable by FE JS at all (same-origin cookie rule, unrelated to CORS/SameSite); the CSRF token is instead returned in the JSON body of `login`/`refresh`/`me` and read from there (`register` only creates the account — no session, no cookies, no `csrfToken` — the client must call `login` separately afterward). `csrfToken` is stored in `localStorage` (`setCsrfToken()`/`getCsrfToken()` in `lib/api/client.ts`), not JS memory or a cookie — it needs to survive a hard reload without an extra round trip and stay in sync across tabs (a refresh rotating it in one tab must be picked up by another), and it isn't a secret that needs XSS protection the way access/refresh tokens are. The deduped refresh lives in `lib/api/client.ts` as `refreshSession()`; `authFetch()` and `ensureSession()` (via `fetchMe()`) share it, so a 401 retry and a session restore never race. `authFetch()` retries once on 401 (refresh session) and once on 403, but only when the 403 body's `message` is exactly the backend's CSRF-failure string (`isCsrfFailure()` checks this via `res.clone().json()`) — re-fetching `/me` to resync a stale `csrfToken` (e.g. rotated in another tab). Any other 403 (wrong registration key, role checks, a staff actor hitting an admin-only carve-out, ...) is returned to the caller as-is, with no `/me` call — both retries are otherwise transparent to the caller. `/api/v1/auth/*` itself is exempt from CSRF checking (login/register can't be forged without the victim's password; refresh/logout are cookie-only and CORS blocks an attacker from reading their responses).
...
**Products are variant-based, not flat.** `Product` only carries identity/copy (`slug`, `name`, `description`, `image`); price, SKU, stock and expiry live on `Product.variants: ProductVariant[]`. There is no more `origin`/`tastingNotes`/`price`/`stockQuantity`/`productTypeId` on `Product` — the backend dropped them along with the `product_type` concept in favor of `product_categories` (flat, no parent/child — `ProductCategory` has no `parentId`) and an EAV `product_properties` model. Use `lib/product-variants.ts` helpers (`getDefaultVariant`, `getPriceRange`, `getTotalStock`) instead of reading a single price/stock field. `Product.categoryIds`/`categoryNames` are returned on every read path (`GET /products`, `/products/:id`, `/products/slug/:slug`, and chat `items`) — `categoryNames` is the display label ("dòng cà phê", e.g. Arabica/Robusta) rendered as a small tag on `ProductCard` (shop grid and `ChatWidget`'s product cards, since the widget reuses `ProductCard`). `GET /api/v1/product-categories` is public (no auth needed) — `shop-content.tsx`'s category filter calls it directly; only `POST`/`PATCH`/`DELETE` on that endpoint are admin/staff-gated. The admin edit dialog still doesn't re-display/preserve existing category assignments on edit (category selection only happens at product creation) — the data is available now, that's just not wired up yet.
